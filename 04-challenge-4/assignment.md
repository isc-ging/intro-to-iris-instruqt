...
- type: text
  contents: |
    ...
    InterSystems IRIS provides built-in support for integrating across a wide range of protocols and formats, including industry-specific and healthcare data formats. This allows you to exchange and transform data between systems without needing additional integration platforms.
...
**In the [Shop](tab-0), find the Gummy Rings, choose a Delivery Location and click "Submit Order".**
...
If you don't see this, or if it has only half loaded, click the refresh button in the top right of the screen:
...
Let's add another component to send the customer a confirmation email. **Select "Create" to add a new component, and choose "Outbound Host" from the dropdown.**
...
To configure this, select:
  ...
  - Name = `ToEmail`
  - Select the "Enable Now" checkbox
